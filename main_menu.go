...
func scanName() {
	color.Cyan("1. Digite el nombre del paquete en singular y CamelCase:")
	fmt.Scan(&n)
	if n == "" {
...
func scanTable() {
	color.Cyan("2. Digite el nombre de la tabla en plural y snake_case:")
	fmt.Scan(&t)
	if t == "" {
...
func scanApi() {
	color.Cyan("4. Digite api si desea que se genere el hanlder para el modelo:")
	fmt.Scan(&h)
}
